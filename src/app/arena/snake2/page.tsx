...
export default function ArenaSnake2Page(){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      try{
        ...
        if(alive){
          if(data.success){
            // nur Übungen verwenden, die Fragen/Blöcke enthalten (für Snake geeignet)
  ...
  return (
    <main className="max-w-6xl mx-auto mt-10 p-6 bg-white rounded shadow">
      <h2 className="text-2xl font-bold mb-4">🐍 Snake 2× (Arena)</h2>
      <p className="text-gray-700 mb-4">Zwei Spieler, ein Board. A: Pfeile • B: WASD. Wähle eine Übung – danach ist die Auswahl gesperrt.</p>
      {!locked && (
        <div className="mb-6">
          {loading ? (
